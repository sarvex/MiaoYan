...
export const listIndent = (
  ...
  if (liElement && previousElement) {
    ...
    liElements.forEach((item: HTMLElement) => {
      ...
      liHTML += `<li data-node-id="${item.getAttribute(
        "data-node-id"
      )}" data-marker="${marker}">${item.innerHTML}</li>`
      item.remove()
    })
...
export const insertRow = (
  ...
  for (let m = 0; m < cellElement.parentElement.childElementCount; m++) {
    rowHTML += `<td align="${cellElement.parentElement.children[
      m
      ].getAttribute("align")}"> </td>`
  }
...
export const insertRowAbove = (
  ...
  for (let m = 0; m < cellElement.parentElement.childElementCount; m++) {
    if (cellElement.tagName === "TH") {
      rowHTML += `<th align="${cellElement.parentElement.children[
        m
        ].getAttribute("align")}"> </th>`
    } else {
      rowHTML += `<td align="${cellElement.parentElement.children[
        m
        ].getAttribute("align")}"> </td>`
          ...
          if (taskItemElement.nextElementSibling) {
            ...
            const dataMarker =
                    taskItemElement.parentElement.tagName === "OL"
                      ? ""
                      : ` data-marker="${taskItemElement.parentElement.getAttribute(
                        "data-marker"
                      )}"`
            let startAttribute = ""
            if (beforeHTML) {
...
export const paste = async (
  ...
  const renderLinkDest: ILuteRenderCallback = (node, entering) => {
    ...
    if (vditor.currentMode === "ir") {
      return [
        `<span class="vditor-ir__marker vditor-ir__marker--link">${src}</span>`,
        Lute.WalkContinue,
      ]
      ...
      return [
        `<span class="vditor-sv__marker--link">${src}</span>`,
        Lute.WalkContinue,
      ]
    }
  }
